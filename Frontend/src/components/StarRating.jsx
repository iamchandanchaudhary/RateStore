...
const StarRating = ({
  ...
  const inactiveClass = disabled ? "text-slate-200" : "text-slate-300";
  const activeClass = disabled ? "text-amber-300/70" : "text-amber-400";
  ...
  const renderStar = (starValue) => {
    ...
    return (
      <span className="relative inline-flex">
        <StarIcon className={`${size} ${inactiveClass}`} />
        <span
          className="absolute inset-0 block overflow-hidden"
          style={{ width: `${fill * 100}%` }}
        >
          <StarIcon className={`${size} ${activeClass} drop-shadow-sm`} />
        </span>
      </span>
    );
  };
